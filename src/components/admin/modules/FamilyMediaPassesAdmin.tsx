...
import { Check, Loader2, Pencil, Plus, RefreshCw, Save, ShieldCheck, X } from "lucide-react";
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [passResponse, familyResponse, seasonResponse, teamResponse] = await Promise.all([
        fetch("/api/admin/family-media-passes?limit=500", { credentials: "same-origin" }),
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 overflow-x-auto">
        {loading ? <p className="flex items-center gap-2 py-6 text-sm font-bold text-slate-500"><Loader2 className="animate-spin" size={18} /> Chargement…</p> : passes.length === 0 ? <p className="rounded-md border border-dashed border-slate-300 p-6 text-center text-sm font-bold text-slate-500">Aucun Pass Famille Média attribué.</p> : (
          <table className="w-full min-w-[900px] border-separate border-spacing-0 text-left text-sm"><thead><tr className="text-xs font-black uppercase text-slate-500">{["Famille", "Saison", "Statut", "Droits", "Équipes", "Validité", ""].map((label) => <th key={label} className="border-b border-slate-200 px-3 py-3">{label}</th>)}</tr></thead><tbody>{passes.map((item) => <tr key={item.id} className="align-top"><td className="border-b border-slate-100 px-3 py-4 font-black text-[#002f1d]">{item.family_name}</td><td className="border-b border-slate-100 px-3 py-4">{item.season_name}</td><td className="border-b border-slate-100 px-3 py-4"><span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-black ${item.status === "ACTIVE" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-700"}`}>{item.status === "ACTIVE" ? <Check size={13} /> : <ShieldCheck size={13} />}{statusLabel(item.status)}</span></td><td className="border-b border-slate-100 px-3 py-4 text-xs font-bold">{[item.allow_photos && "Photos", item.allow_training_videos && "Entraînements", item.allow_live_matches && "Directs"].filter(Boolean).join(" · ")}</td><td className="max-w-[260px] border-b border-slate-100 px-3 py-4 text-xs font-semibold">{item.teams.map((team) => team.name).join(", ")}</td><td className="border-b border-slate-100 px-3 py-4 text-xs font-semibold">{item.starts_on}<br />{item.ends_on}</td><td className="border-b border-slate-100 px-3 py-4"><button type="button" onClick={() => editPass(item)} aria-label={`Modifier ${item.family_name}`} className="focus-ring rounded-md p-2 text-[#07542f] hover:bg-emerald-50"><Pencil size={17} /></button></td></tr>)}</tbody></table>
        )}
      </div>
    </section>
  );
