...
/** Réorganise l'ordre d'affichage des produits de la boutique (glisser-déposer). */
export async function POST(request: NextRequest) {
  ...
  try {
    await reorderRows("products", payload.data.ids);
    await recordActivity({
      actorId: admin.context.user.id,
      action: "shop.product.reordered",
      entityType: "products",
    ...
    return handleDbError("admin/shop/products/reorder", error);
  }
}
